...
(function () {
  ...
  function addImage(image, images, size) {
  ...
  contextPrototype.drawImage = function(image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight) {
    let p = this.getCoords_(dx, dy);
    if (image instanceof CanvasRenderingContext2D_) {
      image.canvas.Images.forEach(function(data) {
        addImage(data, this.canvas.Images, {
          ...
          y: PDFUnit.toFormY(p.y),
          w: PDFUnit.toFormX(sWidth || dWidth),
          h: PDFUnit.toFormY(sHeight || dHeight)
        });
      }, this);
    } else {
